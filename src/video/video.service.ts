  ...
  private buildClipsFromProduct(product: { title: string; description: string; price: string }): VideoClip[] {
    ...
    return [
      ...
      },
      {
        ...
        negativePrompt: [
          sharedNegative,
          'product out of frame, product not visible, product too small, body filling entire frame',
        ].join(', '),
        prompt: [
          `A person's ${bodyParts} are clearly visible actively using "${product.title}".`,
          ...
          `Framing: medium close-up that shows BOTH the product AND the person's ${bodyParts} together in the same shot — neither fills the whole frame alone.`,
          `The product and the interaction with it are equally in focus — the viewer can clearly see what the product looks like AND how it is being used.`,
          `Specifically show: ${action}.`,
          `All movements are slow, smooth, and deliberate — fluid gestures with no jerky or sudden motion.`,
          `Each gesture flows naturally into the next with comfortable pacing.`,
          product.description ? `The demonstration naturally reveals: ${product.description}.` : '',
          `Camera: steady medium close-up, occasionally cutting to a slightly wider shot to show context.`,
