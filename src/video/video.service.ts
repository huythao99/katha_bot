  ...
  private buildClipsFromProduct(product: { title: string; description: string; price: string }): VideoClip[] {
    ...
    return [
      ...
      },
      {
        ...
        negativePrompt: [
          sharedNegative,
          'body parts filling frame, oversized hands, hands too close to camera, body dominating frame',
        ].join(', '),
        prompt: [
          `Close-up product-focused shot: "${product.title}" is the main subject, filling the center of the frame.`,
          ...
          `The person's ${bodyParts} appear partially at the edges or sides of frame — small and secondary — only to show interaction with the product.`,
          `The product occupies at least 60% of the frame at all times. Body parts are visible but never dominant.`,
          `Camera framing: tight product shot — macro or close-up lens — with body parts entering frame from the sides naturally.`,
          ...
          `Camera moves slowly and steadily — smooth dolly or static shot only.`,
